...
namespace MagicTranslatorProject
{
    internal class Translation : DidacticalEnigma.Project.Translation
    {
        ...
        public override IEnumerable<GlossNote> Glosses => Capture.GlossNotes?.Select(c => new GlossNote(c.SideText, c.Text)) ?? Enumerable.Empty<GlossNote>();
            ...
            IEnumerable<TranslatedText> alternativeTranslations = null)
        {
            ...
            other.Capture.GlossNotes = (glosses ?? this.Glosses)?.Select(g => new NoteJson(g.Foreign, g.Text)).ToList() ?? new List<NoteJson>();
